...
function returnPlaySeriesHtml(data,playText) {
    return `
     <div style="text-align: left; color: #fff;">
        ...
        <div style="position: fixed; padding: 20px; height: calc(100vh); overflow: auto; z-index: 2; left: 0; top: 0; width: calc(100vw);">
            <div style="font-size: 0.8em; margin-left: 3px;">${playText}：第${data.Item.ParentIndexNumber}季，第${data.Item.IndexNumber}集</div>
...
function returnPlayMovieHtml(data,playText) {
    return `
    <div style="text-align: left; color: #fff;">
        <img src="${tmdb.imageBaseUrl + data.Tmdb.backdrop_path}" alt="封面" style="height: 100vh; width: 100vw; position: fixed; left: 50vw; top: 0; object-fit: cover; transform: translateX(-50%) scale(1.1); filter: brightness(0.4) blur(10px);">
        <div style="position: fixed; padding: 20px; height: calc(100vh); overflow: auto; z-index: 2; left: 0; top: 0; width: calc(100vw);">
            <div style="font-size: 0.8em; margin-left: 3px;">${playText}</div>
...
function returnAddSeriesHtml(data) {
    return `
        <div style="text-align: left; color: #fff;">
            ...
            <div style="position: fixed; padding: 20px; height: calc(100vh); overflow: auto; z-index: 2; left: 0; top: 0; width: calc(100vw);">
                <div style="font-size: 0.8em; margin-left: 3px;">正在刮削：第${data.eData.season_number}季 第${data.eData.episode_number}集</div>
                ...
                <div style="font-size: 0.8em; margin: 10px auto 0 auto; color: rgba(255, 255, 255, 0.4);">
                        <div>文件地址：${data.data.Item.Path}</div>
...
function returnAddMovieHtml(data) {
   ...
   <div style="text-align: left; color: #fff;">
        <img src="${tmdb.imageBaseUrl + data.Tmdb.backdrop_path}" alt="封面" style="height: 100vh; width: 100vw; position: fixed; left: 50vw; top: 0; object-fit: cover; transform: translateX(-50%) scale(1.1); filter: brightness(0.4) blur(10px);">
        <div style="position: fixed; padding: 20px; height: calc(100vh); overflow: auto; z-index: 2; left: 0; top: 0; width: calc(100vw);">
            <div style="font-size: 0.8em; margin-left: 3px;">正在刮削：</div>
            <div style="font-size: 2em; margin: 10px auto 0 auto; font-weight: 800;">${data.Tmdb.title}</div>
            <div style="font-size: 1em; margin-bottom: 10px; color: rgba(255, 255, 255, 0.6);">${data.Tmdb.original_title}</div>
            <div style="font-size: 0.8em; margin: 0 0 10px 0; color: rgba(255, 255, 255, 0.4);">${data.Tmdb.overview}</div>
            <img src="${tmdb.imageBaseUrl + data.Tmdb.poster_path}" alt="封面" style="width: 100%;">
            <div style="font-size: 0.8em; margin: 10px auto 0 auto; color: rgba(255, 255, 255, 0.4);">
                        <div>文件地址：${data.Item.Path}</div>
                        <div>当前信息为搜索结果，不代表刮削的最终结果</div>
                    </div>
        </div>
    </div>`
